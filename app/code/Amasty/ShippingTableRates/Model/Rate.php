...
class Rate extends AbstractModel
{
    ...
    public function findBy($request, $collection)
    {
            ...
            foreach ($intersectTypes[$key] as $shippingType) {
                ...
                if ($allTotals['qty'] > 0 && (!$this->_scopeConfig->getValue('carriers/amstrates/dont_split') || $allTotals['qty'] == $totals['qty'])) {
                    ...
                    foreach ($this->calculateCosts($allRates, $totals, $request, $shippingType) as $key => $cost) {
                         ...
                         */
                        $method = $this->_objectManager->get('Amasty\ShippingTableRates\Model\Method')->load($key);
        ...
        $method = $this->_objectManager->get('Amasty\ShippingTableRates\Model\Method');
    ...
    protected function getShippingTypes($item)
    {
        ...
        $product = $this->_objectManager->get('Magento\Catalog\Model\Product')->load($item->getProduct()->getEntityId());
    ...
    protected function calculateTotals($request, $ignoreVirtual, $allowFreePromo, $shippingType)
    {
        ...
        foreach ($request->getAllItems() as $item) {
            ...
            if (($item->getHasChildren() && $typeId == 'configurable' && $configurableSetting == '0') ||
                ...
                foreach ($item->getChildren() as $child) {
                    $flagOfPersist = false;
                    $product = $this->_objectManager->get('Magento\Catalog\Model\Product')->load($child->getProduct()->getEntityId());
    ...
    protected function calculateCosts($allRates, $totals, $request, $shippingType)
    {
        ...
        foreach ($allRates as $rate) {
            ...
            if (!$totals['not_free_price'] && !$totals['not_free_qty'] && !$totals['not_free_weight']) {
                ...
                $cost =  $rate['cost_base'] +  $totals['not_free_price'] * $rate['cost_percent'] / 100 + $totals['not_free_qty'] * $rate['cost_product']
                    + $this->calculateWeightCosts($rate, $totals['not_free_weight']);
            }
    ...
    protected function calculateWeightCosts($rate, $weight)
    {
        $weight = floatval($weight);
        $firstWeight = floatval($rate['first_weight']);
        $notFreeWeight = $weight - $firstWeight;
        
        if($firstWeight == 0) {
            return $notFreeWeight * floatval($rate['cost_weight']);
        } else {
            $notFreeWeight = ceil($notFreeWeight/$firstWeight) * $firstWeight;
            return $notFreeWeight/$firstWeight * floatval($rate['cost_weight']);
        }
    }
}
